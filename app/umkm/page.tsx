...
function DataUMKMContent() {
  ...
  const loadUMKM = async () => {
    try {
      setLoading(true)
      console.log("DataUMKM: Loading UMKM for user:", user)

      let data: UMKM[] = []

      if (user?.role === "admin") {
        console.log("DataUMKM: Fetching all UMKM for admin RW:", user.rw)
        data = await umkmService.getAll(undefined, user.rw)
      } else if (user?.id) {
        console.log("DataUMKM: Fetching UMKM for regular user ID:", user.id)
        data = await umkmService.getAll(user.id)
      } else {
        console.log("DataUMKM: No user or role detected, returning empty UMKM list.")
        data = []
      }

      setUmkm(data)
      setFilteredUmkm(data)
      console.log("DataUMKM: UMKM data loaded successfully. Total items:", data.length)
    } catch (error) {
      console.error("DataUMKM: Error loading UMKM:", error)
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleDelete = async (id: string) => {
    if (confirm("Apakah Anda yakin ingin menghapus data UMKM ini?")) {
      try {
        if (!user?.id) {
          alert("Anda harus login untuk menghapus data UMKM.")
          return
        }
        ...
        if (user.role === "admin") {
          ...
          if (umkmToDelete?.user_id) {
            ...
            alert("Tidak dapat menentukan pemilik UMKM untuk dihapus.")
            return
          }
        } else {
  ...
  const exportToCSV = () => {
    const headers = ["Nama Usaha", "Pemilik", "Jenis Usaha", "Status"] // Removed "Nomor HP"
    const csvContent = [
      headers.join(","),
      ...filteredUmkm.map((u) => [u.nama_usaha, u.pemilik, u.jenis_usaha, u.status].join(",")),
    ].join("\n")
          ...
          <CardContent>
            <div className="flex flex-col sm:flex-row gap-4 mb-6">
              <div className="relative flex-1">
                ...
                <Input
                  placeholder="Cari nama usaha, pemilik, jenis usaha..." // Updated placeholder
                  value={searchTerm}
