...
function DashboardContent() {
  ...
  const loadStats = async () => {
    try {
      ...
      if (user?.role === "admin") {
        console.log("Dashboard: Fetching all UMKM for admin RW:", user.rw)
        umkm = await umkmService.getAll(undefined, user.rw)
      } else if (user?.id) {
        console.log("Dashboard: Fetching UMKM for regular user ID:", user.id)
        umkm = await umkmService.getAll(user.id)
      } else {
        console.log("Dashboard: No user or role detected, returning empty UMKM list.")
        umkm = []
      }
