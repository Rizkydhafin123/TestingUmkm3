...
/** Cek apakah string adalah UUID valid atau ID timestamp (untuk fallback lokal) */
export function isValidUUID(str: string | undefined | null): str is string {
  if (!str) return false

  // Jika terhubung ke Neon, hanya terima UUID
  if (hasNeon) {
    return /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(str)
  } else {
    // Jika menggunakan fallback localStorage, terima UUID atau ID timestamp
    // ID timestamp adalah string angka
    return /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(str) || /^\d+$/.test(str)
  }
}
...
export const umkmService = {
  ...
  async update(id: string, payload: Partial<UMKM>, userId: string): Promise<UMKM | null> {
    ...
    // Validasi ID UMKM yang akan diupdate
    if (!isValidUUID(id)) {
      throw new Error("ID UMKM yang akan diupdate tidak valid.")
    }
    // Validasi ketat hanya bila tersambung ke Neon.
  ...
  async delete(id: string, userId: string): Promise<boolean> {
    ...
    // Validasi ID UMKM yang akan dihapus
    if (!isValidUUID(id)) {
      throw new Error("ID UMKM yang akan dihapus tidak valid.")
    }
    // Validasi ketat hanya bila tersambung ke Neon.
    if (hasNeon && !isValidUUID(userId)) {
      throw new Error("User ID tidak valid")
  ...
  async getById(id: string, userId?: string): Promise<UMKM | null> {
    ...
    // Validasi ID UMKM yang akan diambil
    if (!isValidUUID(id)) {
      console.warn(`getById: ID UMKM '${id}' tidak valid.`)
